...
-- 함수는 SELECT문의
-- SELECT 절, WHERE절, ORDER BY절, GROUP BY절, HAVING절 사용 가능
...
-- INSTR(컬렴명 | 문자열, '찾을 문자열' [, 찾기 시작할 위치 [, 순번]])
...
-- MEPLOYEE 테이블에서 사원명, 이메일, 이메일 중 '@' 위치 조회
...
-- 123, 소수점 첫 번째 자리에서 반올림

SELECT ROUND(123.456, 1) FROM DUAL; 
-- 123.5, 소수점 두 번째 자리에서 반올림(소수점 첫번째 자리까지 표기) 
...
-- ADD_MONTHS(날짜, 숫자) : 날짜에 숫자만큼의 개월 수를 더함(음주도 가능)
...
SELECT TO_DATE('490505', 'YYMMDD') FROM DUAL; -- '2049-05-05'
...
FROM EMPLOYEE;
-- 3) TO_CHAR를 이용해서 문자열로 반환 -> 1965년 10월 08일
SELECT EMP_NAME,
...
-- DECODE(계산식 | 컬렴명, 조건값1, 선택값1, 조건값2, 선택값2 ... , 아무것도 일치하지 않을 때)
...
-- 그룹 함수
-- 하나 이상의 행을 그롭으로 묶어 연산하여 총합, 평균 드으이
-- 하나의 결과 행으로 반환하는 함수
