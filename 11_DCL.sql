...
CREATE USER lhg_sample IDENTIFIED BY 1234;
...
-- 3. GRANT 권한, 권한, ... TO 사용자명;
GRANT CREATE SESSION TO lhg_sample;
...
GRANT CREATE TABLE TO lhg_sample;

ALTER USER lhg_sample DEFAULT TABLESPACE SYSTEM QUOTA UNLIMITED ON SYSTEM;
...
GRANT CONNECT, RESOURCE TO lhg_sample;
...
SELECT * FROM ROLE_SYS_PRIVS
WHERE ROLE = 'RESOURCE';
-- CREATE SEQUENCE, CREATE PRCEDURE, CREATE CLUSTER, CREATE INDEXTYPE,
-- CREATE OPERATOR, CREATE TYPE,     CREATE TRIGGER, CREATE TABLE
...
GRANT SELECT ON EMPLOYEE TO lhg_sample;
...
REVOKE SELECT ON EMPLOYEE FROM lhg_sample;
