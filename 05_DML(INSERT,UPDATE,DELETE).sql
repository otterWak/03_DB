...
-- 윈도우 > 설정 > 연결 > 연결유형 Auto commit by default 체크 해제 되어 있는지 확인!
-- 윈도우 > 설정 > 편집기 > SQL 편집기 > SQL 실행 > Blank line is satement delimeter 체크 해제
...
SELECT * FROM EMPLOYEE2
WHERE EMP_ID = '900'; -- 조회 다시해봄(영구저장 되었기 때문에 롤백해도 되돌리기 안됨)
...
-- 서브쿼리(SELERCT) 결과를 EMP_01 테이블에 INSERT
...
-- 5. TRUNCATE (DML 아닙니다 DDL입니다!)
--    테이블의 전체 행을 삭제하는 DDL(Data Definition Language)
--    DELETE 보다 수행속도 더 빠르다
--    ROLLBACK을 통해 복구할 수 없다(COMMIT이 자동으로 되기 때문)
