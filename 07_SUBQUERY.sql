...
-- 1) 직급이 대리인 직원들의 사번, 이름, 직급, 급여 조회 (메인 쿼리)
...
-- 1) LOCATION테이블에서 NATIONAL_CODE가 KO인 경우의 LOCAL_CODE구하기
SELECT LOCAL_CODE
FROM LOCATION
WHERE NATIONAL_CODE = 'KO';

-- 2) DEPARTMENT 테이블의 위의 결과(L1)와 동일한 LOCATION_ID를 가지고 있는 DEPT_ID조회
SELECT DEPT_ID 
FROM DEPARTMENT
...
-- 상관쿼리(이태림이 D8부서에서 가장 빠른 입사&퇴사자여서 걸러짐 - D8부서 아예 제외)
SELECT EMP_ID , EMP_NAME , DEPT_CODE , NVL(DEPT_TITLE, '소속없음') , JOB_NAME , HIRE_DATE
FROM EMPLOYEE MAIN
NATURAL JOIN JOB
LEFT JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
...
--				   중요한 규칙은 USING 절에 명시된 컬럼은 중복되지 않으며, 별칭 없이 바로 사용해야 함
