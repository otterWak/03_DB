...
-- 인원 수 오름차순으로 조회
SELECT DECODE(SUBSTR(EMP_NO, 8,1), '1', '남', '2', '여') 성별, COUNT(*) "인원 수",
SUM(SALARY) "급여 합"
FROM EMPLOYEE
GROUP BY DECODE(SUBSTR(EMP_NO, 8,1), '1', '남', '2', '여') -- 별칭사용 X (SLECET절 해석 전) 
ORDER BY '인워 수'; -- 별칭사용 O (SELECT절 해석 완료)
...
-- * HAVING 절 : 그룹함수로 구해 올 그룹에 대한 조건을 설항할 때 사용
